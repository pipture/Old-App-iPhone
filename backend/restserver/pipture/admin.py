...
def from_local_to_utc_time (tz, time):
    ...
    local_time = datetime.datetime(cur_date.year, cur_date.month, cur_date.day, time.hour, time.minute, time.second)
    utc_time = user_tz.normalize(pytz.utc.localize(local_time)).astimezone(pytz.utc)
    return datetime.time(utc_time.hour, utc_time.minute, utc_time.second)

def from_local_to_utc_date (tz, date):
    user_tz = pytz.timezone(tz)
    local_time = datetime.datetime(date.year, date.month, date.day)
    utc_time = user_tz.normalize(pytz.utc.localize(local_time)).astimezone(pytz.utc)
    return datetime.date(utc_time.year, utc_time.month, utc_time.day)
...
def from_utc_to_local_date (tz, date):
    user_tz = pytz.timezone(tz)
    utc_time = datetime.datetime(date.year, date.month, date.day)
    local_time = pytz.utc.normalize(pytz.utc.localize(utc_time)).astimezone(user_tz)
    return datetime.date(local_time.year, local_time.month, local_time.day)
...
class TimeSlotsAdmin(ButtonableModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        obj.StartDate = from_local_to_utc_date (user_tz, obj.StartDate)
        obj.EndDate = from_local_to_utc_date (user_tz, obj.EndDate)
        obj.save()
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        obj.StartDate = from_utc_to_local_date (user_tz, obj.StartDate)
        obj.EndDate = from_utc_to_local_date (user_tz, obj.EndDate)
        return form    
