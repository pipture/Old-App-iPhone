...
def index (request):
    ...
    response = {
        "Error": {
            "ErrorCode": "888",
            "ErrorDescription": "Unknown API method."
        }
    }
...
#                                StartDate__lt=tomorrow)).order_by('StartTime')
    ...
    for ts in timeslots:
        slot = {
            ...
            "CloseupBackground": ts.AlbumId.CloseUpBackground.get_url()
        }
...
def readSubtitles(subs_url):
    if subs_url is not None and subs_url != "":
            ...
            try:
                ...
                if video != 0:
                    response["Videos"].append({
                        ...
                        "Title": video.Title,
                        "Script": video.Script,
                        "DateReleased": local_date_time_date_time_to_UTC_sec(video.DateReleased),
...
def get_purchased_album_list(userid):
    ...
    try:
        return UserPurchasedItems.objects.filter(UserId=purchaser,
    ...
    if not sallable:
        ...
        try:
            if purchased_albums_list:
                albums_list = Albums.objects.filter(
                    Q(AlbumId__in=purchased_albums_list.values_list("ItemId")) |
        ...
        for album in albums_list:
            ...
            album_each = {
                ...
                'Title': album.Title,
                'ReleaseDate': get_album_status(album)[0],
                'UpdateDate': get_album_status(album)[1],
                'AlbumStatus': get_album_status(album)[2]
            }
        ...
        for album in albums_list:
            if not albumid_inlist(albumid=album.AlbumId, lister=purchased_albums_list) and not album.HiddenAlbum:
                album_each = {
                    'AlbumId': album.AlbumId,
                    'Cover': album.CloseUpBackground.get_url(),
                    'SeriesTitle': album.SeriesId.Title,
                    'Title': album.Title,
                    'SellStatus': Albums.SELL_STATUS_FROM_PURCHASE.get(album.PurchaseStatus, 0)
                }
...
def album_json_by_id (album, purch_list):
    ...
    album_json = {
        ...
        'ReleaseDate': get_album_status(album, get_date_only=True)[0],
        'UpdateDate': get_album_status(album, get_date_only=True)[1]
    }
...
def is_episode_on_air (episode, today):
    ...
    timeslotvideos = TimeSlotVideos.objects.select_related(depth=1)\
                                           .filter(LinkType="E",
...
def enlarge_list(list_data, append_data):
    if list_data is None or append_data is None:
        return
...
def buy (request):
    ...
    #allready bought
    if apple_transaction is not None:
        response["Balance"] = "%s" % purchaser.Balance
        return HttpResponse(json.dumps(response))
    ...
    if apple_product_response == "com.pipture.Pipture.credits":
        ...
        try:
            t = Transactions(UserId=purchaser,
                             ProductId=apple_product,
                             Cost=Decimal(apple_product.Price * apple_product_quantity),
        ...
        ALBUM_EP = PurchaseItems.objects.get(Description="Album")
        new_p = UserPurchasedItems(UserId=purchaser,
                                   ItemId=int(albumid),
                                   PurchaseItemId=ALBUM_EP,
                                   ItemCost=0)
        new_p.save()
...
def deactivateMessageViews (request):
    ...
    try:
        messages = SendMessage.objects.filter(UserId=purchaser)
    except SendMessage.DoesNotExist:
        response["Error"] = {"ErrorCode": "2", "ErrorDescription": "There is no messages for user %s." % purchaser}
        return HttpResponse(json.dumps(response))
